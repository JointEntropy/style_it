...
def load_image(filename, size=None, to_mindim=False):
    """Load and resize an image from disk.

    Inputs:
    - filename: filename or stream
    - size: size of largest dimension after rescaling
    """
    img = imread(filename)
    if size is not None:
        orig_shape = np.array(img.shape[:2])
        min_idx = np.argmin(orig_shape) if to_mindim else np.argmax(orig_shape)
